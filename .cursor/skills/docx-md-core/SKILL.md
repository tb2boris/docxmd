...
Один проход. Исходный `.docx` не изменяется. Реализация — пакет `docxpipe` в каталоге `src`.

## Запуск

Каталог — `src` относительно корня репозитория.
...
- Не подменять этот конвертер другими скриптами Word: у них другой отбор картинок.
